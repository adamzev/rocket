...
class Main_program(object):
    ...
    def __init__(self):
        ...
        self.coast_speed = q.query_float(
            f"Enter the speed at which to end the simulation (or hit enter to select the default speed of {self.coast_speed}): ",
            self.coast_speed,
        )
        ...
        if self.change_weight:
            self.specs["stages"]["orbiter"] = Spec_manager.create_stage_specs(
                "orbiter", "liquid"
            )
            self.specs["MK"] = q.query_string("What is the MK? ")
            self.specs["friendly_name"] = "{} MK {}".format(
                self.specs["name"], self.specs["MK"]
            )
        ...
        try:
            ...
            print(
                "Initial alt, V_vert_target and giveback_target required in spec file."
            )
            raise
    ...
    def compute_row(self, rocket, assigned_A_v, testRun=False):
        ...
        if assigned_A_v == "a" or assigned_A_v == "all":
            ...
            try:
                ...
                print(
                    "Invalid assigned A v. A v = {} A total = {}".format(
                        rocket.cur.A.vert, rocket.cur.A.total
                    )
                )
                raise
...
def restart_menu(last_spec_file=None):
    if last_spec_file:
        ...
        restart_menu_options = ["Restart", "Quit"]
    selection = q.query_from_list(
        "option", "Select an option:", restart_menu_options, False
    )
...
def main():
    while True:
        try:
            ...
            print(
                "A stage ran out of fuel prior to jettisioning. Consider modifying the events."
            )
            restart_menu(Rocketman.specs["file_name"])
