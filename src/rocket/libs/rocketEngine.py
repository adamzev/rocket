...
class RocketEngine(object):
    ...
    def __str__(self):
        return "Name={} Throt={} Eff Burn Rate={} Thrust={} Count={} Stage={}".format(
            self.name,
            self.throt_avg,
            self.get_eff_fuel_burn_rate(),
            self.get_thrust_total(),
            self.engine_count,
            self.stage,
        )
    ...
    def engine_stats(
        ...
        while stat_time < sim_end:
            ...
            if (
                max_reached
                and not min_reached
                and func.less_than_or_almost_equals(self.throt_avg, self.min_throt)
            ):
                min_reached = True
            ...
            stats["throttle_at_end_time"] = (
                self.throt_cur
            )  # keep updating the throttle at end, until the end
            stat_time += time_inc
        return stats
    ...
    def requested_throt_to_float(self, requested_throt):
        ...
        assert func.is_float(requested_throt) or requested_throt in [
            "min",
            "max",
            "off",
        ]
        ...
        if requested_throt >= self.max_throt:
            requested_throt = self.max_throt
        assert (
            func.is_float(requested_throt)
            and requested_throt >= 0.0
            and requested_throt <= 1.0
        )
...
class SolidRocketEngine(RocketEngine):
    ...
    def change_thrust(self, rate, time_inc):
        ...
        if rate > 0:
            message = "\nEVENT: Increased thrust of {} to {}".format(
                self.name, newThrust
            )
        if message:
            self.messages.append(message)
    ...
    def events(self, time, time_inc):
        ...
        if self.event:
            if func.between_floats(
                time, self.event["start_time"], self.event["end_time"]
            ):
                if (
                    self.event["name"] == "Change Thrust"
                    and time < self.event["end_time"]
                ):
    ...
    def power_down_by_burn_rates(self, time, alt, stage):
        ...
        if func.almost_equal(time_since_start, round(time_since_start)):
            try:
                ...
                stage.jettison()
        if stage.attached:
            self.adjust_thrust_to_burn_at_rate_per_engine(
                self.burn_rate_per_engine, alt
            )
...
class LiquidRocketEngine(RocketEngine):
    ...
    def events(self, time, time_inc):
        ...
        if (
            self.power_down_in_progress
            or self.power_down_start_time
            and func.almost_equal(time, self.power_down_start_time, 0.1)
        ):
