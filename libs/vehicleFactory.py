...
class VehicleFactory(object):
	...
	def select_engines(cls, stage_name, stage_jettison_time, fuel_type):
		...
		for engine in compatable_engines:
			...
			if count > 0:
				...
				print("{} {} engines are now attached to the {}.".format(int(count), this_engine["engine_name"], stage_name))
	...
	def collect_engine_details(cls, engine_data, stage_name, stage_jettison_time):
		...
		if starting_throttle == 0.0:
			ignition_time = q.query_float("Engine ignition time (Press enter to select a default value of never): ", float('inf'))
		...
		if stage_name != "orbiter":
			power_down_start_time = q.query_float("{} {} Time to begin throttle down: ".format(
	...
	def collect_goals():
		...
		V_v_giveback_target = q.query_float("What is desired V vert target (typically 1200fps)? ")
	...
	@staticmethod
	def collect_event_details(event):
	...
	def collect_version():
		...
		version_name = q.query_string("What is the version date (hit enter for today's date)? ", today.strftime("%m-%d-%Y"))
		return {
